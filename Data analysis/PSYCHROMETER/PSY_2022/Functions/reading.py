...
def read_Df1(path_input,DF_cols,name): 
    ...
    DF=DF.drop(columns=['Date','Time','Intercept','Slope','IBV','IBT','EPS Present','EPS Voltage','EPS Current','Diagnostic Comment','NA']) # 'EDBO','Correction for dT','Correction Factor',


    DF=DF.apply(lambda x: pd.to_numeric(x, errors='coerce')) #Convert argument to a numeric type.
    DF=DF.astype({'Chamber Temperature':'float','dT':'float','Wet Bulb Depression':'float','Corrected Water Potential':'float','EDBO':'float','Correction for dT':'float','Correction Factor':'float'})#,'Intercept':'float','Slope':'float','EDBO':'float','Correction for dT':'float','Correction Factor':'float'}) #specify the numeric type in this case float 
    DF.columns=['Chamber Temperature (C)','dT (C)','Wet Bulb Depression','Corrected Water Potential (MPa)','EDBO','Correction for dT','Correction Factor']
